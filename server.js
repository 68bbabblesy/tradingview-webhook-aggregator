...
//  G: 1.29 ↔ 3.00 (and negatives)
// ==========================================================

const bot7Last = {}; 
// bot7Last[`${symbol}:${group}`] = { absLevel, rawLevel, time }

function processBot7(symbol, group, ts, body) {
    if (!["G", "H"].includes(group)) return;

    // Get raw signed level from payload
    let raw = NaN;
    ...
    // Only care about these endpoints
    const endpoints = (group === "H") ? [1.29, 1.35] : [1.29, 3.0];
    if (!endpoints.includes(absLevel)) return;

    const key = `${symbol}:${group}`;
    const prev = bot7Last[key];

    // First sighting: store and wait
    if (!prev) {
        bot7Last[key] = { absLevel, rawLevel: raw, time: ts };
        return;
    }

    // Ignore if no abs change
    if (prev.absLevel === absLevel) {
        bot7Last[key] = { absLevel, rawLevel: raw, time: ts }; // refresh time
        return;
    }

    const isTransition =
        (prev.absLevel === endpoints[0] && absLevel === endpoints[1]) ||
        (prev.absLevel === endpoints[1] && absLevel === endpoints[0]);
    ...
    sendToTelegram7(
        `🔁 BOT7 LEVEL TRANSITION\n` +
        `Symbol: ${symbol}\n` +
        `Group: ${group}\n` +
        `From: ${signed(prev.rawLevel)}\n` +
        `To:   ${signed(raw)}\n` +
